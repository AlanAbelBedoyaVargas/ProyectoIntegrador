...
const { Restricciones_alimentaria } = require('../models');

// --- FUNCIÓN PARA CREAR UN NUEVO ALIMENTO ---
const createRestriccion = async (req, res) => {
  try {
    ...
    const nuevaRestriccion = await Restricciones_alimentaria.create({
      nombre,
      descripcion
    });
...
const getAllRestricciones = async (req, res) => {
  try {
    // 7. Usamos el método `findAll` para obtener todos los registros de la tabla 'alimentos'.
    const restricciones = await Restricciones_alimentaria.findAll();
    
    // 8. Devolvemos la lista de alimentos en formato JSON.
    res.status(200).json(restricciones);

  } catch (error) {
    res.status(500).json({ error: 'No se pudieron obtener las restricciones.', details: error.message });
  }
};


// --- FUNCIÓN PARA OBTENER UN ALIMENTO POR SU ID ---
const getRestriccionById = async (req, res) => {
  try {
    ...
    const restriccion = await Restricciones_alimentaria.findByPk(id);
    ...
    res.status(500).json({ error: 'No se pudo obtener la restriccion.', details: error.message });
  }
};


// --- FUNCIÓN PARA ACTUALIZAR UN ALIMENTO ---
const updateRestriccion = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    await Restricciones_alimentaria.update(dataToUpdate);

    // 15. Devolvemos un mensaje de éxito y el objeto del alimento ya actualizado.
    res.status(200).json({ message: 'Alimento actualizado correctamente.', restriccion });
...
const deleteRestriccion = async (req, res) => {
  try {
    const { id } = req.params;

    const restriccion = await Alergia.findByPk(id);
    if (!restriccion) {
